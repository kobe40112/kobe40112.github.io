...
const MemberMyCollection = (props) => {
  ...
  function getDataFromLocalStorage() {
    ...
    const newProduct = JSON.parse(localStorage.getItem('fav')) || []
  ...
  function setDeleteProductToLocalStorage(value) {
    setLoading(true)
    localStorage.setItem('fav', JSON.stringify(value))
    setProduct(value)
    setTimeout(() => {
      setLoading(false)
      handleShowConfirm()
            ...
            return (
              <>
                <Card
                  ...
                  key={item.id}
                >
                  <Card.Header>產品編號：{item.id}</Card.Header>
                  <Card.Img variant="top" src={item.picture} />
                  <Card.Body>
                    <Card.Title className="text-center">{item.name}</Card.Title>
                    <Card.Text style={{height:'11vmin',overflow:'hidden'}}>
                      {item.loninfo}
                    </Card.Text>
                    <div className="row">
                       <button
                        ...
                        onClick={() => {
                          window.location.assign("http://localhost:3000/productList/"+ item.id)
                        }}
                      >
                        去購買
                      </button>
